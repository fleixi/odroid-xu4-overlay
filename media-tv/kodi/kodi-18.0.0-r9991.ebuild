...
IUSE="+airplay alsa bluetooth bluray caps +cec +css dbus debug dvd gles libressl libusb lirc mysql nfs nonfree +opengl pulseaudio +samba sftp systemd +odroid-system-ffmpeg test +udev udisks +odroid-upnp upower vaapi vdpau +webserver -X +xslt +zeroconf mfc wayland systemvolume"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	|| ( gles opengl )
	gles? ( || ( X  wayland ) )
...
COMMON_DEPEND="${PYTHON_DEPS}
	...
	odroid-system-ffmpeg? ( >=media-video/ffmpeg-${FFMPEG_VERSION}:=[encode,postproc] )
	mysql? ( virtual/mysql )
	...
	vdpau? (
		...
		odroid-system-ffmpeg? ( media-video/ffmpeg[vdpau] )
	)
...
EGIT_REPO_URI="git://github.com/MaxKellermann/xbmc.git"
...
pkg_setup() {
	...
	append-ldflags -lwayland-client -lwayland-egl -lxkbcommon -lgbm
}
...
#	if usex systemvolume; then
#		eapply "${FILESDIR}"/${PN}-pulse-systemvolume.patch
#	fi
	eapply "${FILESDIR}"/${PN}-add-mfc-decoder.patch
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DENABLE_INTERNAL_FFMPEG="$(usex !odroid-system-ffmpeg)"
		...
		-DENABLE_UPNP=$(usex odroid-upnp)
		-DENABLE_VAAPI=$(usex vaapi)
		-DENABLE_VDPAU=$(usex vdpau)
		-DENABLE_X11=$(usex X)
		-DENABLE_WAYLAND=$(usex wayland)
		-DENABLE_MFC=$(usex mfc)
...
#		-Dlibdvdcss_URL="${DISTDIR}/libdvdcss-${LIBDVDCSS_COMMIT}.tar.gz"
	...
	use !odroid-system-ffmpeg && mycmakeargs+=( -DFFMPEG_URL="${DISTDIR}/ffmpeg-${PN}-${FFMPEG_VERSION}-${CODENAME}.tar.gz" )
...
src_install() {
	...
	doins "${FILESDIR}"/99-cec-kodi.rules
